...
export function getChoicesRequirements(
  ...
  const fightingStyleRequired =
    (character.class === "guerreiro" && character.level >= 1) ||
    (character.class === "guardiao" && character.level >= 2)
      ? 1
      : 0;
  const fightingStyleOptions = uniqueById(
    feats
      .filter((f) => f.category === "fighting_style")
      .map((f) => ({ id: f.id, name: f.name }))
  );
  const selectedFightingStyle =
    typeof character.classFeatureChoices?.fightingStyleFeatId === "string"
      ? [character.classFeatureChoices.fightingStyleFeatId as string]
      : [];
